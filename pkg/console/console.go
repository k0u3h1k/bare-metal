...
import (
	...
	"github.com/k0u3h1k/bare-metal/pkg/browser"
...
// processToolCalls scans model output for tool calls (shell commands, file ops, network, browse)
...
func processToolCalls(response string, inferenceURL string, maxTokens int) (string, []string) {
	...
	// Detect browse requests
	processed = detectAndExecuteBrowseRequests(processed, &toolResults, inferenceURL, maxTokens)
...
// detectAndExecuteBrowseRequests finds ```browse blocks or URL browsing requests.
//
//	```browse
//	https://example.com
//	```
func detectAndExecuteBrowseRequests(response string, toolResults *[]string, inferenceURL string, maxTokens int) string {
	lines := strings.Split(response, "\n")
	var resultLines []string
	inBlock := false
	var urlBuffer []string

	for _, line := range lines {
		trimmed := strings.TrimSpace(line)

		if !inBlock && (trimmed == "```browse" || trimmed == "```web" || trimmed == "```fetch") {
			inBlock = true
			urlBuffer = nil
			continue
		}
		...
		if inBlock && trimmed == "```" {
			inBlock = false
			urlStr := strings.TrimSpace(strings.Join(urlBuffer, " "))

			if urlStr != "" && browser.CanBrowse(urlStr) {
				// Execute the browse with user permission
				result := executeBrowseWithPermission(urlStr, inferenceURL, maxTokens)
				*toolResults = append(*toolResults, result)
				resultLines = append(resultLines, fmt.Sprintf("_Browsed: %s_", truncate(urlStr, 60)))
			}
			continue
		}

		if inBlock {
			urlBuffer = append(urlBuffer, line)
			continue
		}
...
// executeBrowseWithPermission fetches a URL after getting user consent.
func executeBrowseWithPermission(urlStr string, inferenceURL string, maxTokens int) string {
	// Check permission via the permission system
	req := permissions.Request{
		Action:      permissions.ActionNetwork,
		Description: fmt.Sprintf("Browse URL: %s", urlStr),
		Command:     urlStr,
		Context:     "The model wants to browse the web to gather information.",
	}

	// Check session-level consent first
	if consent, ok := permissions.DefaultSessionTracker.Check(req); ok {
		if consent == permissions.ConsentDenied {
			return fmt.Sprintf("Browse request to '%s' was denied by user (pre-denied via /deny).", urlStr)
		}
		// ConsentAlways — proceed
	} else if !permissions.AllowAll() {
		consent, err := permissions.Prompt(req)
		if err != nil {
			return fmt.Sprintf("Error getting permission: %v", err)
		}
		if consent == permissions.ConsentDenied {
			return fmt.Sprintf("Browse request to '%s' was denied by user.", urlStr)
		}
	}

	// Perform the fetch
	fmt.Printf("\n🌐 Fetching %s...\n", urlStr)
	result, err := browser.Fetch(urlStr)
	if err != nil {
		errMsg := fmt.Sprintf("Error browsing '%s': %v", urlStr, err)
		fmt.Printf("   ⚠️  %s\n", errMsg)
		return errMsg
	}

	summary := result.DescribeContent()
	fmt.Printf("   ✅ %s\n", summary)

	// Build result text for the model
	output := fmt.Sprintf("Content fetched from '%s':\n%s\n\n---\n%s", urlStr, summary, result.Text)
	if result.Truncated {
